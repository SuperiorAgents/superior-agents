	...
	implements ISwapProvider
{
	...
	async _getRoute(params: SwapParams): Promise<any> {
		...
		try {
			...
			const response = await axios.get(
				`${this.baseUrl}/${chainName}/api/v1/routes`,
				{
					headers: {
			...
			if (data.code !== 0) {
				...
				switch (data.code) {
					case 4221:
						throw new Error(
							"WETH token not found. Please verify token addresses.",
						);
					case 4001:
						throw new Error(
							"Invalid query parameters. Please check token addresses and amounts.",
						);
					case 4002:
						throw new Error(
							"Invalid request format. Please verify the input parameters.",
						);
					default:
						throw new Error(
							data.message || "Unknown error occurred while fetching route",
						);
				}
			}
			...
			if (axios.isAxiosError(err)) {
				this.logger.error(
					...
					"Kyber API request failed",
				);

				if (err.response?.data?.code) {
					// If it's a known API error that wasn't caught above
					throw new Error(
						err.response.data.message || "Failed to fetch route from Kyber API",
					);
				}
	...
	async getSwapQuote(params: SwapParams): Promise<SwapQuote> {
		...
		return {
			...
			expectedPrice: new BigNumber(routeSummary.amountOutUsd).dividedBy(
				new BigNumber(routeSummary.amountInUsd),
			),
			fee: fee,
	...
	async getUnsignedTransaction(
		...
		const response = await axios.post(
			`${this.baseUrl}/${chainName}/api/v1/route/build`,
			body,
			{
				headers,
